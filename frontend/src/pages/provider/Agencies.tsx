 ...
 const AGENCIES: Agency[] = [
    {
        ...
        services: [ { id: 1, name: "voiture", duration: 1, price: 23,description: "test", category: { id: 1, name: 'permis' } } ]
    },
    {
        id: 2,
        address: "rue des connards",
    ...
    },
    {
        ...
        services: [ { id: 1, name: "voiture", duration: 1, price: 23,description: "test", category: { id: 1, name: 'permis' } }, { id: 1, name: "camion", duration: 1, price: 23,description: "test", category: { id: 1, name: 'permis' } } ]
    }
]
        ...
        render={() => {
          return (
            <FormItem>
              ...
              <FormControl>
                <SelectMultiple onChange={(ids) => form.setValue('services', ids.map(id => ({ id: Number(id) }) ))} data={SERVICES} placeholder="Select agency where the employee works..."/>
              </FormControl>
              <FormMessage />
            </FormItem>
          )
...
const ModalFormAgency = ({ agency, variant = 'ghost' } : { agency?: Agency, variant?: 'ghost' | 'outline' }) => {
  ...
  return (
    <Dialog onOpenChange={(open) => !open && setIsreadOnly(!!agency)}>
      <DialogTrigger asChild><Button variant={variant} className="px-2">{!agency ? "Add new agency": <PencilIcon />}</Button></DialogTrigger>
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="pb-4">{!agency ? "Add new agency": <>Your agency {isReadOnly && <Button variant={"ghost"} onClick={() => setIsreadOnly(!isReadOnly)} > <PencilIcon /> </Button>} </>}</DialogTitle>
          <DialogDescription>
